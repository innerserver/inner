...
function forceOpenExternalApp(url) {
  try {
...
window.addEventListener("securitypolicyviolation", (event) => {
  if (event.blockedURI && event.blockedURI.startsWith("http")) {
    forceOpenExternalApp(event.blockedURI);
  }
});


function setupEmbeddedAppFallbacks() {
  try {
    if (els.chessFrame) {
      els.chessFrame.addEventListener("error", () => {
        forceOpenExternalApp(currentChessUrl());
      });
      setTimeout(() => {
        try {
          const currentSrc = els.chessFrame.src || "";
          if (!currentSrc.includes("chess")) {
            els.chessFrame.src = currentChessUrl();
          }
        } catch (error) {
          forceOpenExternalApp(currentChessUrl());
        }
    ...
    console.error("Embedded app fallback failed", error);
  }
}
...
function renderEmailStatus() {
  ...
  if (!enabledProviders.length) {
    els.emailStatusText.textContent = "Email alerts need BREVO_API_KEY, RESEND_API_KEY, SENDGRID_API_KEY, or INNER_EMAIL_WEBHOOK_URL in Render.";
    return;
  }
...
/* Hidden tabs + games tab patch */
window.INNER_HIDDEN_TABS = JSON.parse(localStorage.getItem("inner_hidden_tabs") || "[]");
window.INNER_GAMES = JSON.parse(localStorage.getItem("inner_games_links") || "[]");

window.innerAddGameLink = function(name,url){
  const games = window.INNER_GAMES || [];
  games.push({name,url});
  window.INNER_GAMES = games;
  localStorage.setItem("inner_games_links", JSON.stringify(games));
  alert("Game link added");
};

window.innerHideTabForOthers = function(tabId){
  const hidden = new Set(window.INNER_HIDDEN_TABS || []);
  hidden.add(tabId);
  window.INNER_HIDDEN_TABS = [...hidden];
  localStorage.setItem("inner_hidden_tabs", JSON.stringify(window.INNER_HIDDEN_TABS));
  alert("Tab hidden for other users");
};

document.addEventListener("DOMContentLoaded", ()=>{
  try{
    if(!document.getElementById("games-tab-button")){
      const btn=document.createElement("button");
      btn.id="games-tab-button";
      btn.innerText="Games";
      btn.style.margin="8px";
      btn.onclick=()=>{
        const url=prompt("Enter game/app link");
        if(!url) return;
        const name=prompt("Game name") || "Game";
        window.innerAddGameLink(name,url);
...
document.addEventListener("DOMContentLoaded",()=>{
 ...
 const hidden=JSON.parse(localStorage.getItem("privateTabs")||"[]");
 document.querySelectorAll(".nav-button").forEach(btn=>{
   if(hidden.includes(btn.dataset.view)) btn.style.display='none';
   btn.addEventListener('contextmenu',e=>{e.preventDefault(); hidden.push(btn.dataset.view); localStorage.setItem("privateTabs",JSON.stringify(hidden)); btn.style.display='none';});
 });
 function renderGames(){
  gamesList.innerHTML='';
  const games=JSON.parse(localStorage.getItem("games")||"[]");
  games.forEach(g=>{const d=document.createElement('div');d.className='game-card';d.innerHTML=`<strong>${g.name}</strong><br><button onclick="window.open('${g.url}','_blank')" class="primary-button">Open</button>`;gamesList.appendChild(d);});
 }
 if(addBtn){addBtn.onclick=()=>{const n=document.getElementById('gameName').value; const u=document.getElementById('gameUrl').value; const games=JSON.parse(localStorage.getItem("games")||"[]"); games.push({name:n,url:u}); localStorage.setItem("games",JSON.stringify(games)); renderGames();};}
 const toggle=document.getElementById('toggleHiddenButton');
 if(toggle){toggle.onclick=()=>{alert('Right click any sidebar tab to hide it. Clear localStorage privateTabs to restore.');}}
 renderGames();
});


// ===== OWNER FEATURE SYSTEM =====
const OWNER_ACCOUNT = "devshah";
const currentUser = localStorage.getItem("username") || "guest";

const featureLocks = JSON.parse(localStorage.getItem("featureLocks") || "{}");
const hiddenFeatures = JSON.parse(localStorage.getItem("hiddenFeatures") || "{}");
const allowedHiddenUsers = JSON.parse(localStorage.getItem("allowedHiddenUsers") || "{}");

function saveFeatureState(){
  localStorage.setItem("featureLocks", JSON.stringify(featureLocks));
  localStorage.setItem("hiddenFeatures", JSON.stringify(hiddenFeatures));
  localStorage.setItem("allowedHiddenUsers", JSON.stringify(allowedHiddenUsers));
}

window.toggleFeatureHidden = function(feature){
  if(currentUser !== OWNER_ACCOUNT) return;
  hiddenFeatures[feature] = !hiddenFeatures[feature];
  saveFeatureState();
  location.reload();
}

window.toggleFeatureLock = function(feature){
  if(currentUser !== OWNER_ACCOUNT) return;
  featureLocks[feature] = !featureLocks[feature];
  saveFeatureState();
  location.reload();
}

function canAccessFeature(feature){
  if(currentUser === OWNER_ACCOUNT) return true;
  if(hiddenFeatures[feature] && !(allowedHiddenUsers[feature] || []).includes(currentUser)) return false;
  return true;
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  gamesWrap.className = "games-launcher";
  gamesWrap.innerHTML = `
    <div class="games-header">
      <h3>Games</h3>
      <button id="addGameBtn">+</button>
    </div>
    <div id="gamesList"></div>
  `;

  sidebar.appendChild(gamesWrap);
    ...
    games.forEach((game, index) => {
      ...
      card.innerHTML = `
        ...
        <div class="game-actions">
          <button data-open="${index}">Open</button>
          <button data-del="${index}">X</button>
        </div>
      `;
      gamesList.appendChild(card);
    });

    gamesList.querySelectorAll("[data-open]").forEach(btn => {
      btn.onclick = () => {
        const game = games[btn.dataset.open];
        window.open(game.url, "_blank");
      };
    });

    gamesList.querySelectorAll("[data-del]").forEach(btn => {
      btn.onclick = () => {
        games.splice(btn.dataset.del, 1);
  ...
  // chess branding
  const chessPower = document.createElement("div");
  chessPower.className = "chessverse-powered";
  chessPower.innerText = "Powered by ChessVerse";
  sidebar.appendChild(chessPower);

});


// ===== STABLE FEATURE MANAGER PATCH =====
window.__INNER_OWNER__ = "devshah";

(function(){
try{

const OWNER = window.__INNER_OWNER__;
const currentUser = localStorage.getItem("username") || OWNER;

const featureState = JSON.parse(localStorage.getItem("featureState") || "{}");

function saveFeatureState(){
 localStorage.setItem("featureState", JSON.stringify(featureState));
}

window.innerToggleFeature = function(feature, mode){
 ...
 featureState[feature][mode] = !featureState[feature][mode];
 saveFeatureState();
 renderFeatureManager();
};

window.innerAllowFeatureUser = function(){
 const feature = document.getElementById("innerFeatureSelect")?.value;
 const username = document.getElementById("innerAllowedUser")?.value;

 if(!feature || !username) return;
 ...
 if(!featureState[feature].allowed.includes(username)){
   featureState[feature].allowed.push(username);
 }
...
function canUse(feature){
 ...
 if(state.hidden){
   return state.allowed.includes(currentUser);
 }
...
function hideMatchingElements(feature){
...
function disableMatchingElements(feature){
...
function renderFeatureManager(){
 ...
 if(!wrap){
   ...
   wrap.id = "";
   wrap.innerHTML = `
     <div class="innerfm-card">
       <h2>Feature Manager</h2>

       <select id="innerFeatureSelect">
       ...
       <input id="innerAllowedUser" placeholder="Allow username">

       <div class="innerfm-buttons">
         <button onclick="innerToggleFeature(document.getElementById('innerFeatureSelect').value,'hidden')">Hide/Show</button>
         <button onclick="innerToggleFeature(document.getElementById('innerFeatureSelect').value,'locked')">Lock/Unlock</button>
         <button onclick="innerAllowFeatureUser()">Allow User</button>
       </div>
     </div>
   `;
 ...
 ["games","chess","files","chat","admin","ai"].forEach(feature=>{
   hideMatchingElements(feature);
   disableMatchingElements(feature);
 });
...
document.addEventListener("DOMContentLoaded", ()=>{
 ...
 // games fix
 const gamesBtn = document.querySelector("#gamesBtn,.games-btn,[data-feature='games']");
 if(gamesBtn){
   gamesBtn.setAttribute("data-feature","games");
 }

 // chess branding
 if(!document.getElementById("chessverseBrand")){
   const brand = document.createElement("div");
   brand.id = "chessverseBrand";
   brand.innerText = "Powered by ChessVerse";
   brand.style.opacity = ".7";
 ...
 console.error("Feature manager patch failed",e);
}
})();


// ===== ADMIN ONLY PANEL + APP ACCESS SYSTEM =====
(function(){

const OWNER="devshah";
const currentUser=localStorage.getItem("username")||OWNER;
const isAdmin=currentUser===OWNER || localStorage.getItem("role")==="admin";

let appAccess=JSON.parse(localStorage.getItem("appAccess")||"{}");
let appList=JSON.parse(localStorage.getItem("adminApps")||"[]");
let hiddenFeatures=JSON.parse(localStorage.getItem("hiddenFeatures2")||"{}");

function saveAdminState(){
 localStorage.setItem("appAccess",JSON.stringify(appAccess));
 localStorage.setItem("adminApps",JSON.stringify(appList));
 localStorage.setItem("hiddenFeatures2",JSON.stringify(hiddenFeatures));
}

document.addEventListener("DOMContentLoaded",()=>{

 // remove old games tabs
 document.querySelectorAll(".games-tab,#gamesBtn,.games-btn").forEach(e=>e.remove());

 // chess branding only under chess
 const chess=document.querySelector("[data-feature='chess'],#chess,.chess");
 if(chess){
   const brand=document.createElement("div");
   brand.className="cv-brand";
   brand.innerText="Powered by ChessVerse";
   chess.appendChild(brand);
 }

 // admin only feature manager
 ...
 if(isAdmin){
   ...
   function renderAdminApps(){
     ...
     const wrap=document.getElementById("adminAppsList");
     wrap.innerHTML="";
     ...
     wrap.querySelectorAll("[data-open]").forEach(btn=>{
       btn.onclick=()=>{
         const app=appList[btn.dataset.open];
     ...
     wrap.querySelectorAll("[data-del]").forEach(btn=>{
       btn.onclick=()=>{
         appList.splice(btn.dataset.del,1);
         saveAdminState();
         renderAdminApps();
         renderVisibleApps();
       };
   ...
   document.getElementById("addAdminApp").onclick=()=>{

     const name=document.getElementById("adminAppName").value;
     const url=document.getElementById("adminAppURL").value;
     const allowed=document.getElementById("adminAllowedUser").value;

     if(!name || !url || !allowed) return;

     appList.push({name,url,allowed});
   ...
   document.getElementById("toggleHideFeature").onclick=()=>{

     const feature=document.getElementById("hideFeatureSelect").value;
 ...
 function applyHiddenFeatures(){

   Object.keys(hiddenFeatures).forEach(feature=>{

     const hidden=hiddenFeatures[feature];

     document.querySelectorAll(`[data-feature="${feature}"],#${feature},.${feature}`).forEach(el=>{
       el.style.display=hidden && !isAdmin ? "none" : "";
     });
 ...
 function renderVisibleApps(){

   let existing=document.getElementById("allowedAppsContainer");

   if(!existing){
     existing=document.createElement("div");
     existing.id="allowedAppsContainer";
     existing.className="allowed-apps-container";
   ...
   appList.forEach(app=>{
     ...
     if(app.allowed!==currentUser && !isAdmin) return;

     const card=document.createElement("div");
     card.className="allowed-app-card";
...
const isAdmin=username===OWNER || role==="admin";

document.addEventListener("DOMContentLoaded",()=>{

  // HARD REMOVE insecure admin UI for non admins
  if(!isAdmin){

    document.querySelectorAll(
      "#,#,#adminPanel,.admin-panel,.feature-manager"
    ).forEach(el=>el.remove());

    // remove admin buttons/sidebar items
    document.querySelectorAll(
      "[data-feature='admin'],#adminBtn,.admin-btn,.admin-nav"
    ).forEach(el=>el.remove());

    return;
  }

  // secure admin launcher
  let secureBtn=document.getElementById("REMOVED_ADMIN_UI");

  if(!secureBtn){
    secureBtn=document.createElement("button");
    secureBtn.id="REMOVED_ADMIN_UI";
    secureBtn.innerText="REMOVED_ADMIN_UI";
    secureBtn.style.position="fixed";
    secureBtn.style.bottom="20px";
    secureBtn.style.left="20px";
    secureBtn.style.zIndex="999999";
    secureBtn.style.padding="12px 16px";
    secureBtn.style.borderRadius="12px";
    secureBtn.style.border="none";
    secureBtn.style.background="#5b8cff";
    secureBtn.style.color="white";
    secureBtn.style.cursor="pointer";

    document.body.appendChild(secureBtn);
  }

  let panel=document.getElementById("REMOVED_ADMIN_UI");

  if(!panel){

    panel=document.createElement("div");
    panel.id="REMOVED_ADMIN_UI";
  ...
  document.getElementById("closeSecureAdmin").onclick=()=>{
    panel.style.display="none";
  };
...
// ===== FINAL ADMIN / REPORT / SIGNUP PATCH =====
(function(){
...
const role=localStorage.getItem("role")||"user";
const isAdmin=user==="devshah"||role==="admin";

if(!isAdmin){
 document.querySelectorAll(
   "#REMOVED_ADMIN_UI,#REMOVED_ADMIN_UI,#,#"
 ).forEach(e=>e.remove());
}
...
if(isAdmin){
 ...
 let adminTab=document.getElementById("adminPanel");

 if(adminTab){

   const reportWrap=document.createElement("div");
   reportWrap.className="report-admin-wrap";

   reportWrap.innerHTML=`
   <h2>Reports</h2>
   <div id="reportsList"></div>

   <h2>Signup Contacts</h2>
   <div id="signupContacts"></div>
   `;

   adminTab.appendChild(reportWrap);
   ...
   function renderReports(){
     ...
     reports.forEach((r,i)=>{
       ...
       row.innerHTML=`
       <div>
       <b>${r.sender||"Unknown"}</b><br>
       <small>${r.message||"No message"}</small>
       </div>

       <button data-seen="${i}">Mark Seen</button>
       `;
     ...
     list.querySelectorAll("[data-seen]").forEach(btn=>{
       btn.onclick=()=>{
         reports.splice(btn.dataset.seen,1);
   ...
   // signup contacts
   const contacts=JSON.parse(localStorage.getItem("signupContacts")||"[]");
   const contactsWrap=document.getElementById("signupContacts");
...
// TEMP BAN FIX
window.tempBanUser=function(username,minutes){

 let bans=JSON.parse(localStorage.getItem("tempBans")||"{}");

 bans[username]=Date.now()+(minutes*60*1000);

 localStorage.setItem("tempBans",JSON.stringify(bans));

};

window.checkTempBan=function(username){

 let bans=JSON.parse(localStorage.getItem("tempBans")||"{}");

 if(!bans[username]) return false;

 if(Date.now()>bans[username]){
...
// OPEN SIGNUP FIX
window.innerSignup=function(data){
 ...
 let contacts=JSON.parse(localStorage.getItem("signupContacts")||"[]");
 ...
 localStorage.setItem("signupContacts",JSON.stringify(contacts));

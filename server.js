...
function buildRtcConfig() {
  ...
  if (!(relayOnly && turnUrls.length)) {
    ...
    if (!configuredHasStun) iceServers.push({
...
function rtcEnvStatus() {
  return {
    iceJsonKeys: presentEnvKeys("INNER_ICE_SERVERS_JSON", "ICE_SERVERS_JSON", "TURN_ICE_SERVERS_JSON"),
    turnUrlKeys: presentEnvKeys("INNER_TURN_URLS", "TURN_URLS", "TURN_URL", "TURNS_URL", "TURN_SERVER_URLS", "TURN_SERVER_URL", "WEBRTC_TURN_URLS", "RTC_TURN_URLS", "TURNIX_URLS"),
    turnHostKeys: presentEnvKeys("INNER_TURN_HOST", "TURN_HOST", "TURN_SERVER_HOST"),
    turnUsernameKeys: presentEnvKeys("INNER_TURN_USERNAME", "INNER_TURN_USER", "TURN_USERNAME", "TURN_USER", "WEBRTC_TURN_USERNAME", "RTC_TURN_USERNAME", "TURNIX_USERNAME"),
    turnCredentialKeys: presentEnvKeys("INNER_TURN_CREDENTIAL", "INNER_TURN_CREDENTIALS", "INNER_TURN_PASSWORD", "INNER_TURN_SECRET", "TURN_CREDENTIAL", "TURN_CREDENTIALS", "TURN_PASSWORD", "TURN_SECRET", "WEBRTC_TURN_CREDENTIAL", "RTC_TURN_CREDENTIAL", "TURNIX_CREDENTIAL", "TURNIX_PASSWORD"),
  };
}

function presentEnvKeys(...names) {
...
function isPrivateBrowserAddress(address) {
...
function isPrivateBrowserHostname(hostname) {
  ...
  if (net.isIP(host)) return isPrivateBrowserAddress(host);
  return false;
}

async function assertSafeBrowserTarget(target) {
  ...
  if (isPrivateBrowserHostname(parsed.hostname)) throw new Error("Local and private network targets are blocked.");

  const addresses = await dns.promises.lookup(parsed.hostname, { all: true, verbatim: true });
  if (!addresses.length || addresses.some((entry) => isPrivateBrowserAddress(entry.address))) {
    throw new Error("Local and private network targets are blocked.");
  }
}

async function guardedBrowserFetch(target, redirectsLeft = 5) {
  await assertSafeBrowserTarget(target);
  const parsed = new URL(target);
  ...
  return new Promise((resolve, reject) => {
    const req = transport.request(parsed, {
      ...
      lookup(hostname, options, callback) {
        dns.lookup(hostname, options, (error, address, family) => {
          ...
          if (isPrivateBrowserAddress(address)) return callback(new Error("Local and private network targets are blocked."));
      ...
      if ([301, 302, 303, 307, 308].includes(status) && location) {
        ...
        try {
          ...
          await assertSafeBrowserTarget(next);
          resolve(guardedBrowserFetch(next, redirectsLeft - 1));
        } catch (error) {
          reject(error);
        }
        return;
      ...
      upstream.on("data", (chunk) => {
        total += chunk.length;
        if (total > maxBytes) {
          req.destroy(new Error("Browser response was too large."));
          return;
        }
        chunks.push(chunk);
      });
    ...
    req.on("timeout", () => req.destroy(new Error("Browser request timed out.")));
    req.on("error", reject);
    req.end();
  });
}
...
async function serveBrowserFrame(req, res, requestUrl, user) {
  ...
  try {
    ...
    const response = await guardedBrowserFetch(target);
    const contentType = String(response.headers["content-type"] || "text/html; charset=utf-8");
    ...
    const status = /credentials|local|private|only http|redirect/i.test(message) ? 403 : 502;
    return browserFrameMessage(res, status, "Connectifi browser could not open this site", `${message}\n\nIf your network blocks this site, Connectifi cannot bypass that.`);
  }
}
